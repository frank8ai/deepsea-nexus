...
- RAG integration
- Context management (auto-summary, session, flush)
"""
...
# Export context management modules
...
class DeepSeaNexus:
    ...
    Usage:
        ...
        # Context management
        session = nexus.start_session("topic")
    ...
    @property
    def auto_summary(self) -> HybridStorage:
        """Get auto-summary storage."""
        if self._auto_summary is None:
            self._ensure_initialized()
            self._auto_summary = HybridStorage(self._manager)
        return self._auto_summary

    def start_session(self, topic: str) -> str:
        """Start a new session."""
        return self.session_manager.start_session(topic)

    def end_session(self, session_id: str):
        """End a session."""
        return self.session_manager.close_session(session_id)

    def process_with_summary(self, conversation_id: str, response: str) -> Dict[str, Any]:
        """Process response with auto-summary."""
        result = self.auto_summary.process_and_store(
            conversation_id=conversation_id,
            response=response
        )
        return result

    def daily_flush(self) -> Dict[str, Any]:
        """Execute daily flush."""
        flush_mgr = FlushManager()
        return flush_mgr.daily_flush(self.session_manager)
...
if __name__ == "__main__":
    ...
    print(f"  Sessions: {stats['session_manager']}")
