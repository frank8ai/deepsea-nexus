...
## Metadata
- Scorecard ID: SCORE-20260217-30
- SOP ID: SOP-20260217-30
- Date: 2026-02-17
- Owner: yizhi
- Constraints summary: 把需求从“描述”升级为“可验收交付”，避免上线后返工和口径不一致。

## Candidate Options
| Option | Description | Notes |
|---|---|---|
| A | 三段评审（需求澄清 -> 约束核对 -> 验收走查）。 | 在质量与效率间平衡 |
...
|---|---|---|---|---|
| 需求评审必须绑定验收用例与不可协商约束。 | PRISMA 2020:https://www.bmj.com/content/372/bmj.n71 | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |
| 需求评审必须绑定验收用例与不可协商约束。 | NIST Information Quality:https://www.nist.gov/director/nist-information-quality-standards | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |

## Best Method Decision
- Selected method: Option A (三段评审（需求澄清 -> 约束核对 -> 验收走查）。)
...
|---|---|---|---|---|
| PRD模板 | 核心执行 | 评审漏项率下降 >=30%；返工率下降 >=25% | 工具依赖增加 | 模板字段裁剪；看板回退到最小字段 |
