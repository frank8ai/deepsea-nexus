...
## Metadata
- Scorecard ID: SCORE-20260217-40
- SOP ID: SOP-20260217-40
- Date: 2026-02-17
- Owner: yizhi
- Constraints summary: 在主网发布前用硬门禁拦截高风险合约缺陷和权限漏洞。

## Candidate Options
| Option | Description | Notes |
|---|---|---|
| A | 预发布清单 -> 安全测试 -> 权限演练 -> 上线审批。 | 在质量与效率间平衡 |
...
|---|---|---|---|---|
| 合约上线必须满足审计、权限、暂停与升级四重门禁。 | OpenZeppelin Contracts:https://docs.openzeppelin.com/contracts/5.x/ | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |
| 合约上线必须满足审计、权限、暂停与升级四重门禁。 | Solidity Security Considerations:https://docs.soliditylang.org/en/latest/security-considerations.html | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |

## Best Method Decision
- Selected method: Option A (预发布清单 -> 安全测试 -> 权限演练 -> 上线审批。)
...
|---|---|---|---|---|
| Slither/Echidna | 核心执行 | 高危漏洞上线率下降 >=50%；权限配置错误下降 >=40% | 工具依赖增加 | 触发pause并切回上一稳定合约版本 |
