...
## Metadata
- Scorecard ID: SCORE-20260217-41
- SOP ID: SOP-20260217-41
- Date: 2026-02-17
- Owner: yizhi
- Constraints summary: 在链上安全事件中最短时间止损、取证并统一外部沟通。

## Candidate Options
| Option | Description | Notes |
|---|---|---|
| A | 检测 -> 暂停/限流 -> 公告 -> 取证 -> 恢复 -> 复盘。 | 在质量与效率间平衡 |
...
|---|---|---|---|---|
| 安全事件响应必须先控制资金风险，再发布证据化沟通。 | NIST Incident Handling:https://csrc.nist.gov/pubs/sp/800/61/r2/final | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |
| 安全事件响应必须先控制资金风险，再发布证据化沟通。 | OpenZeppelin Defender Monitor:https://docs.openzeppelin.com/defender/monitor | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |

## Best Method Decision
- Selected method: Option A (检测 -> 暂停/限流 -> 公告 -> 取证 -> 恢复 -> 复盘。)
...
|---|---|---|---|---|
| 链上监控 | 核心执行 | 止损时间下降 >=40%；误操作率下降 >=30% | 工具依赖增加 | 冻结高风险路径并回退到应急模式 |
