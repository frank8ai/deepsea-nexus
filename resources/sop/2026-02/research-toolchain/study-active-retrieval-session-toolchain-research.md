...
- Date: 2026-02-17
- SOP: `resources/sop/2026-02/2026-02-17-study-active-retrieval-session-sop.md`
- SOP Name: 学习主动检索会话
...
- Best Practice evidence rows: 检索练习优于重读 <- https://pubmed.ncbi.nlm.nih.gov/21252317/；Test-enhanced learning <- https://pubmed.ncbi.nlm.nih.gov/16719566/；闭环记录 <- agent/patterns/sop-factory.md
- Best Method score: Winner B=4.40, Runner-up=3.80, Margin=0.60, Hard constraints=passed
- Best Tool evidence: gain[题库/闪卡:回忆率提升 >=25%；计时器:单次效率提升 >=20%；会话日志:错因闭环率提升 >=30%], rollback[题库/闪卡->每周校准题库；计时器->放宽窗口；会话日志->最小字段记录]
