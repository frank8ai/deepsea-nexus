...
- Date: 2026-02-17
- SOP: `resources/sop/2026-02/2026-02-17-p1-weekly-monthly-review-sop.md`
- SOP Name: 周月复盘与规则更新
...
- Best Practice evidence rows: 闭环学习原则 <- SOP_PRINCIPLES.md；1-3规则限制 <- agent/patterns/sop-factory.md；strict校验 <- scripts/validate_sop_factory.py
- Best Method score: Winner B=4.40, Runner-up=3.80, Margin=0.60, Hard constraints=passed
- Best Tool evidence: gain[复盘模板:复盘质量提升 >=30%；指标看板:偏差定位速度提升 >=25%；规则库:复用率提升 >=20%], rollback[复盘模板->强制偏差分析；指标看板->指标分层；规则库->定期清理]

## Three-Optimal Conclusion
- 最佳实践: 复盘必须产出可执行规则
- 最佳方法: 指标对比+偏差归因+小步规则更新
- 最佳工具: 复盘模板+指标看板+规则库
