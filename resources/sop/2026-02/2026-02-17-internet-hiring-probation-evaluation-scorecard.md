...
## Metadata
- Scorecard ID: SCORE-20260217-37
- SOP ID: SOP-20260217-37
- Date: 2026-02-17
- Owner: yizhi
- Constraints summary: 降低招聘随机性，提升岗位匹配度与试用期成功率。

## Candidate Options
| Option | Description | Notes |
|---|---|---|
| A | 岗位画像 -> 结构化面试 -> 试用期里程碑评估。 | 在质量与效率间平衡 |
...
|---|---|---|---|---|
| 面试必须结构化评分，试用期必须目标化评估。 | NIST Information Quality:https://www.nist.gov/director/nist-information-quality-standards | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |
| 面试必须结构化评分，试用期必须目标化评估。 | PRISMA 2020:https://www.bmj.com/content/372/bmj.n71 | source-backed | 降低关键失败风险 | 执行僵化或成本上升 |

## Best Method Decision
- Selected method: Option A (岗位画像 -> 结构化面试 -> 试用期里程碑评估。)
...
|---|---|---|---|---|
| 评分卡模板 | 核心执行 | 误判率下降 >=20%；试用期达标率提升 >=15% | 工具依赖增加 | 回退到上一版题库和评分标准并复校 |
